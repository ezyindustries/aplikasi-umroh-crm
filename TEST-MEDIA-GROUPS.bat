...
curl -s -o nul -w "Media endpoint: %%{http_code}\n" http://localhost:3003/api/messages/media/test
echo.
echo Testing dashboard endpoint...
curl -s http://localhost:3003/api/dashboard/test | node -pe "try { const data = JSON.parse(require('fs').readFileSync(0, 'utf-8')); console.log('Database stats:', JSON.stringify(data.data, null, 2)); } catch(e) { console.log('❌ Backend not accessible'); }"
